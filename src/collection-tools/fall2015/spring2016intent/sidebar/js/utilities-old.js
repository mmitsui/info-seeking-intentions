...
var loadedobjects = "";
var rootdomain = "http://"+window.location.hostname+"/spring2016intent/";
...
function inviteCollab() {
	var inviteEmail = document.getElementById('inviteEmail').value;
	var sureInvite = document.getElementById('sureInvite');
	sureInvite.innerHTML = 'Are you sure you want to add <span style="font-weight:bold">'+inviteEmail+ '</span> to this project?<br/><a href="http://www.coagmento.org/CSpace/addCollaborator.php?targetUserName='+inviteEmail+'">Yes</a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0);" onClick="cancelInvite();">No</a>';
}
...
function createProj() {
	...
	if (document.getElementById('public').checked)
		...
		projPrivacy = 1;
	var sureCreate = document.getElementById('sureCreate');
	sureCreate.innerHTML = 'Are you sure you want to create project <span style="font-weight:bold">'+projTitle+ '</span>?<br/><a href="http://www.coagmento.org/CSpace/createProject.php?title='+projTitle+'&description='+projDesc+'&privacy='+projPrivacy+'">Yes</a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0);" onClick="cancelCreateProj();">No</a>';
}
...
function addAction (action, value) {
	req = new phpRequest("http://www.coagmento.org/CSpace/timelineview/addAction.php");
...
function showBookmark(ID,parentID,bookmarkID,type) {
    ...
    sText = sText + "<p>Note: "+document.getElementById("note"+bookmarkID).value.substr(0,20)+"...</p>";
//    if (type == "text")
...
function showQuery(ID,parentID,queryID,type) {
    // var sText = "<center><table style=\"text-align: center; color: blue; font-weight: bold\" width: 100%;\" border=\"0\" cellpadding=\"0\" cellspacing=\"10\"><tr><td>";
    var sText = "";
    sText = sText + "<p>Source: "+document.getElementById("querysource"+queryID).value.substr(0,200)+"...</p><hr>";
...
function showBookmarkRatingForm(ID,parentID,itemID,type,region,webPage) {
    var currentY = cY;
    var typeAux = "snippet";
    if (type=="pages"){
        typeAux = "page";
    ...
    sText = sText + "<td><input type=\"radio\" id=\"rating3\" name=\"rating\" onclick=\"javascript:saveBookmarkRating('"+ID+"','"+itemID+"','"+type+"','"+region+"','"+webPage+"')\" value=\"3\"></td>";
    sText = sText + "<td><input type=\"radio\" id=\"rating4\" name=\"rating\" onclick=\"javascript:saveBookmarkRating('"+ID+"','"+itemID+"','"+type+"','"+region+"','"+webPage+"')\" value=\"4\"></td>";
    sText = sText + "<td><input type=\"radio\" id=\"rating5\" name=\"rating\" onclick=\"javascript:saveBookmarkRating('"+ID+"','"+itemID+"','"+type+"','"+region+"','"+webPage+"')\" value=\"5\"></td></tr>";
    sText = sText + "<tr align=center><td>1</td><td>2</td><td>3</td><td>4</td><td>5</td></tr></table>";
    sText = sText + "<br><br>";
    sText = sText + "<BUTTON onclick=\"javascript:hideLayer('"+ID+"')\"><STRONG>Cancel</STRONG></BUTTON></center>";
...
function saveBookmarkRating(ID,itemID,type,region,webPage)
{
	...
	if (value != 0)
	{
		ajaxpage('sidebarComponents/updateBookmarkRating.php?type='+type+'&itemID='+itemID+'&value='+value+'&webPage='+webPage, region);
		//ajaxpage('sidebarComponents/insertAction.php?action=saveRating_'+type+'&value='+itemID,null);
		/*
         new Ajax.Request('insertAction.php', {
         method: 'post',
...
function saveBookmarkRatingSimple(itemID,type, value)
{
    ajaxpage('updateBookmarkRating.php?type='+type+'&itemID='+itemID+'&value='+value+'&webPage=',null);
}
