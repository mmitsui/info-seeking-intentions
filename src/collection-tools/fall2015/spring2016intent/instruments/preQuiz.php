		...
		if (isset($_POST['preQuiz'])) 
		{
			...
			foreach($_POST as $key=>$value){
				if (strpos($key,'group')!==FALSE && strcmp($key,'groupcheckbox')!=0){
					...
					$groupresponse = $_POST['group'.$groupnum];
					
					$query = "INSERT INTO user_quiz_preresponses (userID, quizID,topicAreaID, preResponseID)
									VALUES('$userID','$groupnum','$topicAreaID','$groupresponse')";
...
<center>
	<br/>
	<form action="preQuiz.php" method="post" onsubmit="return validate(this)">
		<table class="body" width="90%">
		<tr><th><span style="font-weight:bold; font-size:20px">Exploratory Search User Study: TASK (Pre Quiz)</span><br/><br/></th></tr>
		<tr><td><hr/></td></tr>
				
			<tr>
			<td>
				<ul>
				<li>Please select answers for ALL of the following questions related to the area of interest you selected before from the multiple choices.</li>					
				<li>Please refrain from searching the answers to these questions on the internet or any other media and answer based on your current knowledge.</li>
				<li>Your performance evaluation would not depend on the number of correct answers you give to this set of questions, so please attempt to answer honestly based on your current domain knowledge. </li>
				<li>Once you have selected answers to ALL questions to the best of your knowledge, please click 'Continue' to proceed. </li>
				<li>Then you would be shown a topic and a task based on the area you selected that you would have to work on using the system for 1-2 hours.</li>	
				</ul>
			</td>
		</tr>
		<tr><td><hr/></td></tr>	
		...
		<input type="hidden" name="preQuiz" value="true"/><input type="submit" value="Continue" />
		</td>
		</tr>
	  </table>
	</form>
